...
{
	public class NightRule : IOrderRule
	{
		private const string c_strPotato = "potato";
		...
		public bool Apply(IRepository<IDishType> repo, /*List<IDishRequest> drRequests,*/ ref List<IDishDisplay> ddOutput)
		{
			...
			//are either morning or all items are night, if the first item is a morning meal then this rule does not apply.

			if (ddOutput[0].TypeRef.MealTime != MealTimeEnum.Night)
				return true;

			bool result = true;		//We're going to set this to false as soon as we have a failure condition

			//We know this is a night meal now, let's apply the logic as defined:
			//"For the night meal you can order multiple sides of potato but every other item may be ordered only once"
			...
			IDishType potato = repo.GetByName(c_strPotato);
			Debug.Assert(potato != null);	//If this is null then there's a real problem. Getting null here isn't what I'd
											//test with a normal isnull check.

			foreach (IDishDisplay item in ddOutput)
			{
				if (item.TypeRef.Id != potato.Id)
					if (item.Count > 1)
					{
						result = false;
						item.IsInError = true;
