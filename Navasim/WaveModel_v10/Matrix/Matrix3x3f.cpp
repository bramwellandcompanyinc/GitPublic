...
Matrix3x3f::Matrix3x3f(float fp00, float fp01, float fp02,
...
void Matrix3x3f::Init(float fp00, float fp01, float fp02,
					  float fp10, float fp11, float fp12,
					  float fp20, float fp21, float fp22)
{
	m_fpElements[0] = fp00;
...
float Matrix3x3f::CalculateDeterminant()
{
	float fpRes;
	Matrix2x2f m22A, m22B, m22C;
...
Matrix3x3f& Matrix3x3f::Copy(Matrix3x3f* ap_m33In, Matrix3x3f* ap_m33Out)
{
	...
	//It would be nice to implement this copy as a set of compiler intrinsics... REP STOSQW (=8 bytes = 2 floats so count 4) and then a etc...
	...
	float* p_fpEi = ap_m33In->m_fpElements;
	float* p_fpEo = ap_m33Out->m_fpElements;
...
Matrix3x3f& Matrix3x3f::Transpose()
{
	...
	float* fpE = m_fpElements;
	Matrix3x3f m33(fpE[0], fpE[3], fpE[6],  fpE[1], fpE[5], fpE[7],  fpE[2], fpE[6], fpE[8]);
...
void Matrix3x3f::Multiply(Matrix3x3f& a_m33Left, Matrix3x3f& a_m33Right, Matrix3x3f& a_m33Out)
{
	float* p_fpL = a_m33Left.m_fpElements;
	float* p_fpR = a_m33Right.m_fpElements;
	float* p_fpO = a_m33Out.m_fpElements;
...
Matrix3x3f& Matrix3x3f::RotateInWorldX(float fpAngle)
{
	...
	float fpSinX = sin(fpAngle);
	float fpCosX = cos(fpAngle);

	//We could eventually define a static rotation matrix for the 4 types - x, y, z, & vector rotations

	Matrix3x3f m33Out;
	Matrix3x3f m33RotX(1,0,0,  0,fpCosX, -fpSinX,  0,fpSinX,fpCosX);
...
Matrix3x3f& Matrix3x3f::RotateInWorldY(float fpAngle)
{
	float fpSinY = sin(fpAngle);
	float fpCosY = cos(fpAngle);

	Matrix3x3f m33Out;
	Matrix3x3f m33RotY(fpCosY,0,fpSinY,  0,1,0,  -fpSinY,0,fpCosY);
...
Matrix3x3f& Matrix3x3f::RotateInWorldZ(float fpAngle)
{
	float fpSinZ = sin(fpAngle);
	float fpCosZ = cos(fpAngle);

	Matrix3x3f m33Out;
	Matrix3x3f m33RotZ(fpCosZ,-fpSinZ,0,  fpSinZ,fpCosZ,0,  0,0,1);
	Matrix3x3f::Multiply(*this, m33RotZ, m33Out);
	Copy(&m33Out, this);
...
Point3f& Matrix3x3f::Transform(Point3f& p3In, Point3f& p3Out)
{
...
void Matrix3x3f::Transform(float* p_fpXIn, float* p_fpYIn, float* p_fpZIn, float* p_fpXOut, float* p_fpYOut, float* p_fpZOut, Int64 i64CountPoints)
{
